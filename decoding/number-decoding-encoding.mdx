...
`,
  ...
  pythonHeight="36rem"
  pythonCode={`def decode_number(hex_string: str) -> int:
    # Convert the hex string to bytes
    element = bytes.fromhex(hex_string)
    ...
    # Initialize result and check if the number is negative
    result = 0
    negative = big_endian[0] & 0x80
    
    # Set the initial result based on the sign
    if negative:
        result = big_endian[0] & 0x7F  # Use the absolute value for negative
    else:
        result = big_endian[0]

    # Build the result from the remaining bytes
    for i in range(1, len(big_endian)):
        result <<= 8
        result += big_endian[i]

    # Return the final result, negating if it's negative
