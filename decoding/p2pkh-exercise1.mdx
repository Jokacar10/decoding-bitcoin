...
export default P2PKHGame;`
  ...
  pythonHeight="40rem"
  pythonCode={`
from p2pkh_exercise import construct_p2pkh_scriptpubkey
...
for i, public_key in enumerate(public_keys):
    result = construct_p2pkh_scriptpubkey(public_key)
    print(f"Public Key {i+1}:")
    print(f"Result:   {result}")
    print(f"Expected: {expected_scriptpubkeys[i]}")
    print(f"Correct:  {result == expected_scriptpubkeys[i]}\\n")
    # Test data
public_keys = [
    '037d91c138583e25b0dbacb2643e5f14a96df6af798b3dcc08d7f7f3b41db614fb',
    '02f82f8cd17a73ea73538ba86d129254c9ef640babc51b316e089595bb884dbef5'
...
for i, public_key in enumerate(public_keys):
    ...
    print(f"Correct:  {result == expected_scriptpubkeys[i]}\\n")`}
  pythonModules={{
    "p2pkh_exercise.py": `import hashlib

def construct_p2pkh_scriptpubkey(public_key: str) -> str:
    # TODO: Implement the P2PKH scriptPubKey construction
    # The function should take a public key as input and return the P2PKH scriptPubKey in hex format
    # Steps:
    # 1. Convert the public key from hex to bytes
    # 2. Hash the public key using SHA256 and then RIPEMD160
    # 3. Construct the script: OP_DUP OP_HASH160 <pubKeyHash> OP_EQUALVERIFY OP_CHECKSIG
