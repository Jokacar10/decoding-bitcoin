...
OP_CODE_NAMES = {
    ...
    0x59: 'OP_9',
    0x5a: 'OP_10',
    # Add other opcode mappings as necessary
}

def parse_script(script_hex: str):
    # Remove '0x' prefix if it exists and convert hex string to bytes
    script = bytes.fromhex(script_hex.replace('0x', ''))
    parsed_script = []
    offset = 0
    ...
    while offset < len(script):
        ...
        if 0x01 <= opcode <= 0x4b:
            ...
            data_length = opcode
            data = script[offset:offset + data_length]
            parsed_script.append({'type': 'data', 'value': data.hex()})
            offset += data_length

        elif opcode == OpCode.OP_PUSHDATA1:
            data_length = script[offset]
            offset += 1
            data = script[offset:offset + data_length]
            parsed_script.append({'type': 'data', 'value': data.hex()})
            offset += data_length
            ...
            data_length = struct.unpack('<H', script[offset:offset + 2])[0]  # Read 2 bytes as little-endian unsigned short
            offset += 2
            data = script[offset:offset + data_length]
            parsed_script.append({'type': 'data', 'value': data.hex()})
            offset += data_length
            ...
            data_length = struct.unpack('<I', script[offset:offset + 4])[0]  # Read 4 bytes as little-endian unsigned int
            offset += 4
            data = script[offset:offset + data_length]
            parsed_script.append({'type': 'data', 'value': data.hex()})
            offset += data_length
            ...
            # Opcode
            opcode_name = OP_CODE_NAMES.get(opcode, f'UNKNOWN_{opcode:02x}')
            parsed_script.append({'type': 'opcode', 'value': opcode_name})
